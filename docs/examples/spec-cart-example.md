...
```text
/speckit.specify
購入者が商品をカートに追加し、購入前に内容を確認・編集できる機能。
...
前提: カタログ機能が実装済み。消費税は一律10%（軽減税率対象外）。
...
src/domains/cart/ の暫定スキャフォールド（@/samples/ を再エクスポート中）を本番実装に置き換える。src/contracts/ の共有インターフェースに準拠し、src/samples/domains/cart/ は参考実装として参照する。
