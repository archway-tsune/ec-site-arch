...
全操作はadmin認証が必須で、buyer権限のユーザーはアクセスを拒否される。

【商品一覧表示（管理者）】
管理者が全商品をテーブル形式で確認できる（商品名・価格・ステータス）。
各商品に対して編集・削除の操作ができる。
...
前提: 購入者向けのカタログ閲覧機能（catalog ドメイン）は実装済み。
...
src/domains/ の暫定スキャフォールドを本番実装に置き換える構成とする。src/contracts/ の共有インターフェースに準拠し、src/samples/domains/catalog/ の CRUD 部分は参考実装として参照する。
